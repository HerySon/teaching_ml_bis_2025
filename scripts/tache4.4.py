"""
Module tache4_4
Ce module propose une fonction pour tester différentes méthodes de plongement
non linéaire (Isomap, MDS, t-SNE, UMAP, etc.) afin de visualiser les données
en 2D. L'objectif est d'explorer la structure du jeu de données de manière
plus intuitive qu'avec un simple PCA linéaire.
"""
...
def reduire_dimension(
    ...
    """
    Réduit la dimension d'un DataFrame pour une visualisation en 2D
    à l'aide d'une méthode de plongement non linéaire.

    Args:
        df (pd.DataFrame): Données d'entrée (uniquement colonnes numériques).
        method (str): Méthode parmi "isomap", "mds", "tsne", "umap".
        n_components (int): Dimension cible (2 pour la visualisation).
        random_state (int): Graine aléatoire pour la reproductibilité.

    Returns:
        pd.DataFrame: Un DataFrame contenant les coordonnées 2D résultantes,
                      avec les mêmes index que df.
    """
    ...
    if method == "isomap":
        ...
        raise ValueError("Méthode inconnue. Choisir parmi "
                         "'isomap', 'mds', 'tsne', 'umap'.")

    # Application de la réduction
    embedding = model.fit_transform(df_num)

    # Construction d'un DataFrame pour les coordonnées 2D
    columns = [f"comp_{i+1}" for i in range(n_components)]
    df_embedded = pd.DataFrame(embedding, columns=columns, index=df_num.index)
...
def main() -> None:
    ...
    """
    # Exemple de données (4 individus, 5 features dont 1 catégorielle ignorée)
    data = {
    ...
    # Test avec UMAP (nécessite umap-learn)
